...
export default function DataTable({ columns, data }) {
    ...
    const mesesDisponiveis = [...new Set(data.map(item => {
        const date = new Date(item.data);
    ...
    const calcularTotal = () => {
        const total = dadosFiltrados.reduce((accumulator, currentValue) => accumulator + parseFloat(currentValue.valor), 0);
                    ...
                    <DropdownMenuContent className="w-56">
                        ...
                        <DropdownMenuGroup>
                            {mesesDisponiveis.map((mes, index) => (
                                <DropdownMenuItem key={index} onClick={() => filtrarPorMes(mes)}>{mes}</DropdownMenuItem>
                            ))}
                        </DropdownMenuGroup>
                    </DropdownMenuContent>
                </DropdownMenu>
